...
def app_home_page(request):
    ...
    return render(request, 'shopify_app/homepage.html', data)


def boboandmusu_store_view(request):
    data = {
        'title': 'Bobo and Musu'
    }
    print(request.headers)
    print(request.GET)
    res_render = render(request, 'home/boboandmusu_store_view.html')
    res_render['Content-Type'] = 'application/liquid'
    res_render['Mime-Type'] = 'application/liquid'
    return res_render


class AppFingerprintList(ListView):
    model = ShopifyCustomerFingerPrint
    queryset = ShopifyCustomerFingerPrint.objects.all().order_by('-created')
    template_name = 'shopify_app/fingerprints/fingerprint_list.html'
    context_object_name = 'fingerpritns'
    paginate_by = 20
