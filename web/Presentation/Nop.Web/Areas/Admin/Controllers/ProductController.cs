...
namespace Nop.Web.Areas.Admin.Controllers
{
  public partial class ProductController : BaseAdminController
  {
    ...
    #region Product Model Templates

    [HttpPost]
    public virtual IActionResult ProductModelTemplateList(ProductModelTemplateSearchModel searchModel)
    {
      if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
        return AccessDeniedKendoGridJson();
      ...
      //prepare model
      var model = _productModelFactory.PrepareProductModelTemplateListModel(searchModel, product);

      return Json(model);
    }

    [HttpPost]
    public virtual IActionResult ProductModelTemplateUpdate(ProductModelTemplateModel model)
    {
      if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
        return AccessDeniedView();

      var productModelTemplate = _productService.GetProductModelTemplateById(model.Id)
                           ?? throw new ArgumentException("No product model template found with the specified id");

      //a vendor should have access only to his products
      if (_workContext.CurrentVendor != null)
      {
        var product = _productService.GetProductById(productModelTemplate.ProductId);
        if (product != null && product.VendorId != _workContext.CurrentVendor.Id)
          return Content("This is not your product");
      }


      productModelTemplate.Active = model.Active;
      productModelTemplate.UpdatedDate = DateTime.UtcNow;
      _productService.UpdateProductModelTemplate(productModelTemplate);

      return new NullJsonResult();
    }

    [HttpPost]
    public virtual IActionResult ProductModelTemplateAdd(ProductModelTemplateModel model)
    {
      if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
        return AccessDeniedView();
      ...
      _productService.InsertProductModelTemplate(new ProductModelTemplate
      {
        ProductId = model.ProductId,
        ModelTemplatePath = model.ModelTemplatePath,
        ModelTemplateTypeId = model.ModelTemplateTypeId,
        Active = false,
    ...
    [HttpPost]
    public virtual IActionResult ProductModelTemplateDelete(int id)
    {
      if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
        return AccessDeniedView();

      var productModelTemplate = _productService.GetProductModelTemplateById(id)
                           ?? throw new ArgumentException("No product model template found with the specified id");
      ...
      _productService.DeleteProductModelTemplate(productModelTemplate);
    ...
    public virtual IActionResult AsyncModelTemplateUpload(int productId)
    {
      ...
      var rootPath = Path.Combine(PrintPath.ROOT_MODEL_TEMPLATES, productId.ToString()); //$"Themes/BoBoMuSuTheme/Content/ModelTemplates/{productId}";
      var rootRelativePath = Path.Combine(PrintPath.RELATIVE_MODEL_TEMPLATES, productId.ToString()); //$"Themes/BoBoMuSuTheme/Content/ModelTemplates/{productId}";
      ...
      var modelPath = "";
      var modelPhysicalPath = FileExtensions.GetUniqueFilePath(Path.Combine(rootPhysicalFolder, $"{httpPostedFile.FileName}"));
      modelPath = $"{rootRelativePath}/{Path.GetFileName(modelPhysicalPath)}";
      ...
      return Json(new { success = true, id = 1, modelPath = modelPath });
    }
