...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public virtual ProductModel PrepareProductModel(ProductModel model, Product product, bool excludeProperties = false)
        {
            ...
            if(product != null)
                model.AddModelTemplateModel = new ProductModelTemplateModel()
                {
                    Upload = new ProductModelTemplateModel.UploadModel()
                    {
        ...
        public virtual ProductModelTemplateListModel PrepareProductModelTemplateListModel(ProductModelTemplateSearchModel searchModel, Product product)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            var productModelTemplates = _productService.GetProductModelTemplatesByProductId(product.Id);

            //prepare grid model
            var model = new ProductModelTemplateListModel
            {
                Data = productModelTemplates.PaginationByRequestModel(searchModel).Select(productModelTemplate =>
                {
                    //fill in model values from the entity
                    var productModelTemplateModel = new ProductModelTemplateModel
                    {
                        Id = productModelTemplate.Id,
                        ProductId = productModelTemplate.ProductId,
                        ModelTemplateFileName = Path.GetFileName(productModelTemplate.ModelTemplatePath),
                        ModelTemplateTypeId = productModelTemplate.ModelTemplateTypeId,
                        ModelTemplateTypeName = ((ModelTemplateType)productModelTemplate.ModelTemplateTypeId).ToString(),
                        ModelTemplateUrl = "/../Admin/BMDownload?type=modeltemplate&id=" + productModelTemplate.Id,
                        Active = productModelTemplate.Active
                    };
                    
                    return productModelTemplateModel;
                }),
                Total = productModelTemplates.Count
            };
