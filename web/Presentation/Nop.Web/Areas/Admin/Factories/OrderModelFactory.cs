    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        protected virtual void PrepareOrderItemModels(IList<OrderItemModel> models, Order order)
        {
            ...
            foreach (var orderItem in orderItems)
            {
                ...
                orderItemModel.PrintOrderItems = new List<PrintOrderItemModel>();
                orderItem.PrintOrderItems.ForEach(m =>
                {
                    var productPicture = _productService.GetProductPictureById(m.ProductPictureId ?? -1);
                    var picture = productPicture!=null ? _pictureService.GetPictureById(productPicture.PictureId) : null;
                    orderItemModel.PrintOrderItems.Add(new PrintOrderItemModel()
                    {
                        Id = m.Id,
                        ...
                        Print = new PrintModel()
                        {
                            Id = m.Print.Id,
                            Name = m.Print.Name,
                            ThumbnailPath = m.Print.ThumbnailPath,
                        },
                        ModelPath = m.ModelPath,
                        ProductPictureThumbnailUrl = picture != null ? _pictureService.GetPictureUrl(picture, 300, false) : ""
                    });
                });
