...
namespace Nop.Web.Controllers
{
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        public ShoppingCartController(CaptchaSettings captchaSettings,
            ...
            IAuthenticationService authenticationService, IPrintModelFactory printModelFactory)
        {
        ...
        protected virtual void SaveItem(ShoppingCartItem updatecartitem, List<string> addToCartWarnings, Product product,
           ...
           DateTime? rentalEndDate, int quantity, List<PrintProductPicture> printProductPictures)
        {
            if (updatecartitem == null)
            {
                ...
                addToCartWarnings.AddRange(_shoppingCartService.AddToCart(_workContext.CurrentCustomer,
                    ...
                    rentalStartDate, rentalEndDate, quantity, true, printProductPictures: printProductPictures));
            }
            else
            {
                ...
                addToCartWarnings.AddRange(_shoppingCartService.UpdateShoppingCartItem(_workContext.CurrentCustomer,
                    ...
                    rentalStartDate, rentalEndDate, quantity + (otherCartItemWithSameParameters?.Quantity ?? 0), true, printProductPictures: printProductPictures));
            ...
            int quantity, bool forceredirection = false)
        {
            ...
            // Redirect to add Print
            if (product.ProductModelTemplates.Any(m => m.Active))
            {
                return Json(new
                {
        ...
        public virtual IActionResult AddProductToCart_Details(int productId, int shoppingCartTypeId, IFormCollection form)
        {
            ...
            var printProductPictures = new List<PrintProductPicture>();
            if (form.Keys.Contains("add-print-to-cart-select"))
            {
                //printId_productpictureId
                var items = form["add-print-to-cart-select"].ToString().Split(',');
                foreach (var item in items)
                {
                    if (string.IsNullOrEmpty(item)) continue;
                    var temps = item.Split('_');
                    printProductPictures.Add(new PrintProductPicture()
                    {
                        PrintId = Convert.ToInt32(temps[0]),
                        ProductPictureId = Convert.ToInt32(temps[1]) > 0? (int?)Convert.ToInt32(temps[1]) : null
                    });
                }
            }
            else if (form.Keys.Contains("printId"))
            {
                printProductPictures.Add(new PrintProductPicture()
                {
                    PrintId = Convert.ToInt32(form["printId"])
                });
            }
            SaveItem(updatecartitem, addToCartWarnings, product, cartType, attributes, customerEnteredPriceConverted, rentalStartDate, rentalEndDate, quantity, printProductPictures);
        ...
        public virtual IActionResult UpdateCart(IFormCollection form)
        {
            ...
            var warnings = orderedCart.Select(cartItem => new
            {
                ...
                Warnings = _shoppingCartService.UpdateShoppingCartItem(_workContext.CurrentCustomer,
                    ...
                    printProductPictures: cartItem.Item.PrintShoppingCartItems.Select(m=> new PrintProductPicture()
                    {
                        PrintId = m.PrintId,
