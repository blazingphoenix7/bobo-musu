...
namespace Nop.Web.Factories
{
  public partial class CatalogModelFactory : ICatalogModelFactory
  {
    ...
    public virtual TopMenuModel PrepareTopMenuModel()
    {
      //categories
      var cachedCategoriesModel = PrepareCategorySimpleModels();
      var newCategories = new List<CategorySimpleModel>();
      var topMenuItem = "";
      if (_workContext.CurrentCustomer.IsRegistered())
      {
        ...
        if (requests != null
           && requests.Count > 0)
        { messageCounts = requests.Count; }

        string msgCountString = messageCounts == 0 ? string.Empty : $" ({messageCounts})";
        topMenuItem = $"Messages{msgCountString}";
        newCategories.Add(new CategorySimpleModel()
        {
          Name = topMenuItem,
          IncludeInTopMenu = true,
          DisplayOrder = 5,
          SeName = "print/messagelist"
        });
        topMenuItem = "Help & Faqs";
        newCategories.Add(new CategorySimpleModel()
        {
          Name = topMenuItem,
          IncludeInTopMenu = true,
          ...
          SubCategories = new List<CategorySimpleModel>()
                    {
                        ...
                        {
                            Name = "My Prints",
                            SeName = "print/myprints"
                        },
                        new CategorySimpleModel()
                        {
                            Name = "Shared Prints",
                            SeName = "print/sharedprints"
                        },
                        new CategorySimpleModel()
                        {
                            Name = "Request a Print",
                            SeName = "print/askforprint"
                        },
                    },
          DisplayOrder = 10,
          SeName = "print/myprints"
          ...
          SubCategories = new List<CategorySimpleModel>()
                    {
                        ...
                        {
                            Name = "My Information",
                            SeName = "customer/info"
                        },
                        new CategorySimpleModel()
                        {
                            Name = "Addresses",
                            SeName = "customer/addresses"
                        },
                        new CategorySimpleModel()
                        {
                            Name = "Orders",
                            SeName = "order/history"
                        },
                        new CategorySimpleModel()
                        {
                            Name = "Change Password",
                            SeName = "customer/changepassword"
                        }
                    },
          DisplayOrder = 40,
          SeName = "customer/info"
      ...
      else // Remove if existed
      {
        var topMenuItemNames = new List<string>()
                {
                    "Saved Items",
                    "Cart",
                    "Register",
        ...
        foreach (var topMenuItemName in topMenuItemNames)
        {
          var existedCategory = cachedCategoriesModel.FirstOrDefault(m => m.Name == topMenuItemName);
          if (existedCategory != null)
          {
            cachedCategoriesModel.Remove(existedCategory);
          }
        }
      }
      ...
      newCategories = newCategories.OrderBy(m => m.DisplayOrder).ToList();
      foreach (var newCategory in newCategories)
      {
        var existedCategory = cachedCategoriesModel.FirstOrDefault(m => m.Name == newCategory.Name);
        if (existedCategory != null)
        {
          cachedCategoriesModel.Remove(existedCategory);
        }
        cachedCategoriesModel.Add(newCategory);
      }
      ...
      var model = new TopMenuModel
      {
        ...
        LoginCustomerId = _workContext.CurrentCustomer.Id
      };
      return model;
    }
