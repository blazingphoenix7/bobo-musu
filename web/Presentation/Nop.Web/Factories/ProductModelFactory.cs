...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual ProductDetailsModel.AddToCartModel PrepareProductAddToCartModel(Product product, ShoppingCartItem updatecartitem)
        {
            ...
            var myPrints = _printService.GetPrints(_workContext.CurrentCustomer.Id);
            model.Prints = myPrints.Select(m => new PrintModel()
            {
                Id = m.Id,
                Name = m.Name,
            ...
            var sharedPrints = _printService.GetSharedPrints(_workContext.CurrentCustomer.Id);
            sharedPrints.ForEach(p =>
            {
                if (model.Prints.Count(m => m.Id == p.Id) == 0)
                {
                    model.Prints.Add(new PrintModel()
                    {
                        Id = p.Id,
                        Name = p.Name,
        ...
        /// <param name="updatecartitem">Updated shopping cart item</param>
        /// <returns>Product add to cart model</returns>
        protected virtual ProductDetailsModel.AddPrintToCartModel PrepareProductAddPrintToCartModel(Product product, ShoppingCartItem updatecartitem)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));


            var prints = _printService.GetPrints(_workContext.CurrentCustomer.Id);
            var model = new ProductDetailsModel.AddPrintToCartModel
            {
                ProductId = product.Id,
                Prints = prints.Select(m=>new PrintModel()
                {
                    Id = m.Id,
                    Name = m.Name,
                    ThumbnailPath = m.ThumbnailPath
        ...
        protected virtual ProductPictureModel PrepareProductDetailsPictureModel(Product product, bool isAssociatedProduct, out IList<ProductPictureModel> allPictureModels)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                var defaultPictureModel = new ProductPictureModel
                {
                ...
                //all pictures
                var pictureModels = new List<ProductPictureModel>();
                foreach (var picture in pictures)
                {   
                    var pictureModel = new ProductPictureModel
                    {
                    ...
                    var productPicture = product.ProductPictures.FirstOrDefault(m => m.PictureId == picture.Id);
                    if (productPicture != null)
                        pictureModel.ProductPictureId = productPicture.Id;
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            model.DefaultPictureModel = PrepareProductDetailsPictureModel(product, isAssociatedProduct, out IList<ProductPictureModel> allPictureModels);
