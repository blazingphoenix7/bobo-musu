...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        private readonly IRepository<ProductModelTemplate> _productModelTemplateRepository;
        ...
        public ProductService(CatalogSettings catalogSettings,
            ...
            IRepository<ProductModelTemplate> productModelTemplateRepository)
        {
            ...
            _productModelTemplateRepository = productModelTemplateRepository;
        ...
        public virtual IList<ProductModelTemplate> GetProductModelTemplatesByProductId(int productId)
        {
            var query = from pp in _productModelTemplateRepository.Table
                where pp.ProductId == productId
                orderby pp.Id
                select pp;
            var productModelTemplates = query.ToList();
            return productModelTemplates;
        }

        public virtual ProductModelTemplate GetProductModelTemplateById(int productModelTemplateId)
        {
            if (productModelTemplateId == 0)
                return null;

            return _productModelTemplateRepository.GetById(productModelTemplateId);
        }

        public void InsertProductModelTemplate(ProductModelTemplate productModelTemplate)
        {
            if (productModelTemplate == null)
                throw new ArgumentNullException(nameof(productModelTemplate));

            _productModelTemplateRepository.Insert(productModelTemplate);

            //event notification
            _eventPublisher.EntityInserted(productModelTemplate);
        }

        public void UpdateProductModelTemplate(ProductModelTemplate productModelTemplate)
        {
            if (productModelTemplate == null)
                throw new ArgumentNullException(nameof(productModelTemplate));

            _productModelTemplateRepository.Update(productModelTemplate);

            //event notification
            _eventPublisher.EntityUpdated(productModelTemplate);
        }

        public void DeleteProductModelTemplate(ProductModelTemplate productModelTemplate)
        {
            if (productModelTemplate == null)
                throw new ArgumentNullException(nameof(productModelTemplate));

            _productModelTemplateRepository.Delete(productModelTemplate);

            //event notification
            _eventPublisher.EntityDeleted(productModelTemplate);
        }
