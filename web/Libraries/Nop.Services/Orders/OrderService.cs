...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        public virtual void DeleteOrderItem(OrderItem orderItem)
        {
            ...
            var printOrderItems = _printOrderItemRepository.Table.Where(m => m.OrderItemId == orderItem.Id).ToList();
            for (var i = 0; i < printOrderItems.Count; i++)
            {
                _printOrderItemRepository.Delete(printOrderItems[i]);
            }
        ...
        public void InsertPrintOrderItem(PrintOrderItem printOrderItem)
        {
            if (printOrderItem == null)
                throw new ArgumentNullException(nameof(printOrderItem));

            _printOrderItemRepository.Insert(printOrderItem);
        }

        public void UpdatePrintOrderItem(PrintOrderItem printOrderItem)
        {
            if (printOrderItem == null)
                throw new ArgumentNullException(nameof(printOrderItem));

            _printOrderItemRepository.Update(printOrderItem);
        }
