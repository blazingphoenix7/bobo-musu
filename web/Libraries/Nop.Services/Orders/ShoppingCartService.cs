...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        private readonly IRepository<PrintShoppingCartItem> _psciRepository;
        ...
        public ShoppingCartService(CatalogSettings catalogSettings,
            ...
            IRepository<PrintShoppingCartItem> psciRepository)
        {
            ...
            _psciRepository = psciRepository;
        }
            ...
            bool ensureOnlyActiveCheckoutAttributes = false)
        {
            ...
            //delete item
            var printShoppingCartItems = _psciRepository.Table.Where(m => m.ShoppingCartItemId == shoppingCartItem.Id).ToList();
            for (var i = 0; i < printShoppingCartItems.Count; i++)
            {
                _psciRepository.Delete(printShoppingCartItems[i]);
            }
        ...
        /// <param name="quantity">Quantity</param>
        /// <param name="addRequiredProducts">Whether to add required products</param>
        /// <param name="printProductPictures">printProductPictures</param>
        /// <returns>Warnings</returns>
        public virtual IList<string> AddToCart(Customer customer, Product product,
            ShoppingCartType shoppingCartType, int storeId, string attributesXml = null,
            decimal customerEnteredPrice = decimal.Zero,
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null,
            int quantity = 1, bool addRequiredProducts = true, List<PrintProductPicture> printProductPictures = null)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            if (shoppingCartItem != null)
            {
                ...
                var newQuantity = quantity; // Just add 1
                ...
                var printShoppingCartItems = _psciRepository.Table.Where(m=>m.ShoppingCartItemId == shoppingCartItem.Id).ToList();
                if (printProductPictures != null)
                {
                    for (var i = 0; i < printProductPictures.Count; i++)
                    {
                        var printProductPicture = printProductPictures[i];
                        if (printShoppingCartItems.Count(m => m.PrintId == printProductPicture.PrintId && m.ProductPictureId == printProductPicture.ProductPictureId) == 0)
                        {
                            var printShoppingCartItem = new PrintShoppingCartItem()
                            {
                                ShoppingCartItemId = shoppingCartItem.Id,
                                PrintId = printProductPicture.PrintId,
                                ProductPictureId = printProductPicture.ProductPictureId,
                                Price = 0,
                                CreatedDate = DateTime.UtcNow,
                                UpdatedDate = DateTime.UtcNow
                            };
                            _psciRepository.Insert(printShoppingCartItem);
                            shoppingCartItem.PrintShoppingCartItems.Add(printShoppingCartItem);
                        }
                    }
                }
                for (var i = 0; i < printShoppingCartItems.Count; i++)
                {
                    if (printProductPictures == null || !printProductPictures.Any(m=> m.PrintId == printShoppingCartItems[i].PrintId && m.ProductPictureId == printShoppingCartItems[i].ProductPictureId))
                    {
                        _psciRepository.Delete(printShoppingCartItems[i]);
                        shoppingCartItem.PrintShoppingCartItems.Remove(printShoppingCartItems[i]);
                    }
                }
        ...
        public virtual IList<string> UpdateShoppingCartItem(Customer customer,
            ...
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null,
            int quantity = 1, bool resetCheckoutData = true, List<PrintProductPicture> printProductPictures = null)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            if (quantity > 0)
            {
                ...
                for (var i = 0; i < printShoppingCartItems.Count; i++)
                {   
                    if (printProductPictures == null || !printProductPictures.Any(m=> m.PrintId == printShoppingCartItems[i].PrintId 
                    && m.ProductPictureId == printShoppingCartItems[i].ProductPictureId))
                    {
                        _psciRepository.Delete(printShoppingCartItems[i]);
                    }
                }
        ...
        public void UpdatePrintShoppingCartItem(PrintShoppingCartItem printShoppingCartItem)
        {
            _psciRepository.Update(printShoppingCartItem);
        }
